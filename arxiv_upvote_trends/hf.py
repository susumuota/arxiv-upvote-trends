...
def _get_huggingface(api: HfApi, date: str, wait: float = 1.0) -> list[dict]:
    ...
    papers = []
    p = 0
    while len(papers) < _MAX_DAILY_PAPERS:
        time.sleep(wait)
        page_papers = list(api.list_daily_papers(date=date, limit=_PAGE_SIZE, p=p))
        logger.info("Fetched %s papers for date %s page %s", len(page_papers), date, p)
        papers.extend(page_papers)
        if len(page_papers) < _PAGE_SIZE:
            break
        p += 1
    return [asdict(paper) for paper in papers[:_MAX_DAILY_PAPERS]]
...
def search_huggingface(max_papers: int = 300, days: int = 30, wait: float = 1.0) -> list[dict]:
    ...
    papers = []
    for date in dates:
        papers.extend(_get_huggingface(api, date=date, wait=wait))
        if len(papers) >= max_papers:
            break
    return deduplicate(papers, key=_paper_id)[:max_papers]


def _paper_id(paper: dict) -> str | None:
    return str(paper.get("id") or "") or None
